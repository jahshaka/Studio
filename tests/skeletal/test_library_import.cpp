...
int main(int argc, char **argv)
{
    ...
    // ---- the rig, through the aiScene overload ----------------------------
    {
        ...
        const QVector<QMatrix4x4> bind = mn->getSkeleton()->boneTransforms;
        doc->updateSceneAnimation(0.5f);
        const QVector<QMatrix4x4> posed = mn->getSkeleton()->boneTransforms;
        int nonIdentity = 0;
        for (const auto &m : posed) if (!m.isIdentity()) ++nonIdentity;
        std::printf("    non-identity skin matrices at t=0.5: %d/%lld\n",
                    nonIdentity, (long long)posed.size());
        // THE assertion. Before the fix this was 0 of 2, forever, with no warning.
        CHECK(nonIdentity >= 1, "at least one skin matrix is non-identity at t > 0");
        CHECK(posed != bind, "the pose actually changed between t=0 and t=0.5");
        ...
        if (!dup.isNull()) {
            ...
            if (!dupMesh.isNull()) {
                auto dupHost = findClipHost(dup);
                if (dupHost) dupHost->setAnimation(dupHost->getAnimations().first());
                mn->updateAnimation(0.5f);
                if (dupHost) dupHost->updateAnimation(0.0f);
                CHECK(dupMesh->getSkeleton()->boneTransforms != mn->getSkeleton()->boneTransforms,
                      "two library copies of one rig hold different poses");
            }
        }
    }
