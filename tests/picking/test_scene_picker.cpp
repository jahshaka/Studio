...
static iris::MeshNodePtr cubeAt(iris::ScenePtr doc, const iris::Vec3 &pos, const char *name, iris::SceneNodePtr parent = nullptr) {
    ...
    n->setName(name);
    n->setMesh(previewmesh::load(":assets/models/cube.obj"));
...
int main(int argc, char **argv) {
    ...
    auto front = cubeAt(doc, iris::Vec3(0, 0, 0), "front");
    ...
    // mesh hits report their triangle (V-hold vertex snap reads its corners)
    {
        ...
        const iris::Mat4 xf = front->getGlobalTransform();
        float nearest = 1e9f;
        for (const iris::Vec3 &c : { xf * tri.a, xf * tri.b, xf * tri.c })
    ...
    auto back = cubeAt(doc, iris::Vec3(0, 0, -4), "back");
    ScenePicker::screenSegment(cam, W, H, QPointF(W / 2, H / 2), a, b);
    ...
    front->setLocalPos(iris::Vec3(10, 0, 0));       // move the front one away
    hits = ScenePicker::pickAll(doc, a, b, cam->getGlobalPosition());
    CHECK(ScenePicker::nearest(hits).node == back, "after moving it, the back cube is picked");
    front->setLocalPos(iris::Vec3(0, 0, 0));
    ...
    light->setLocalPos(iris::Vec3(-2.5f, 0, 0));
    ...
    light->setLocalPos(iris::Vec3(0, 0, 2.5f));      // between camera and cube
    hits = ScenePicker::pickAll(doc, a, b, cam->getGlobalPosition());
    ...
    decal->setLocalPos(iris::Vec3(-2.5f, 0, 0));
    ...
    auto sibling = cubeAt(doc, iris::Vec3(2, 0, 0), "sibling", holder);
    ...
    // obvious: it is pickable from one side and not the other.
    {
        ...
        tm.addTriangle(iris::Vec3(-1, 0, -1), iris::Vec3(-1, 0, 1), iris::Vec3(1, 0, -1));

        QList<iris::TriangleIntersectionResult> r;
        const int fromAbove = tm.getSegmentIntersections(iris::Vec3(-0.5f, 1, -0.5f),
                                                         iris::Vec3(-0.5f, -1, -0.5f), r);
        ...
        r.clear();
        const int fromBelow = tm.getSegmentIntersections(iris::Vec3(-0.5f, -1, -0.5f),
                                                         iris::Vec3(-0.5f, 1, -0.5f), r);
        ...
        r.clear();
        CHECK(tm.getSegmentIntersections(iris::Vec3(-0.5f, 1, -0.5f),
                                         iris::Vec3(0.5f, 1, 0.5f), r) == 0,
    ...
    // ...and end to end, through the picker: a plane picked from underneath.
    {
        ...
        pnode->setMesh(previewmesh::load(QStringLiteral(JAHSHAKA_SOURCE_DIR "/app/content/primitives/plane.obj")));
        pdoc->getRootNode()->addChild(pnode);
        pdoc->getRootNode()->update(0.0f);

        const iris::Vec3 above(0.05f, 5, 0.05f), below(0.05f, -5, 0.05f);
        ...
        CHECK(ScenePicker::pickAll(pdoc, iris::Vec3(500, 5, 500), iris::Vec3(500, -5, 500),
                                   iris::Vec3(500, 5, 500)).isEmpty(),
    ...
    // ortho(-s..+s) convention), so clicks land where the pixels are.
    {
        ...
        ocam->setLocalPos(iris::Vec3(0, 10, 0));
        ocam->setLocalRot(iris::Quat::fromEulerAngles(-90, 0, 0)); // straight down
        ...
        auto east = cubeAt(odoc, iris::Vec3(4, 0, 0), "east");
        auto west = cubeAt(odoc, iris::Vec3(-4, 0, 0), "west");

        iris::Vec3 a1, b1, a2, b2;
        ...
        const iris::Vec3 d1 = (b1 - a1).normalized(), d2 = (b2 - a2).normalized();
        CHECK(iris::Vec3::dotProduct(d1, d2) > 0.9999f, "ortho rays are parallel (not eye-fanned)");
