...
// reads. And the RAY HIT's read of a point against the march's spelling of the
// same read.
//
...
static std::vector<VoxelReaderCone> makeCones(unsigned cascades)
{
...
{
    const std::vector<VoxelReaderCone> cones = makeCones(cascades);
    ...
    CHECK(hitDiffer == 0u, (std::string(label) + ": the ray hit's read of a point IS the "
                            "march's read of it, bit for bit").c_str());
...
int main()
{
    ...
    // THE SHIPPED CHAIN: anisotropic, four cascades.
    {
        ...
        measureChain(e, scene, "anisotropic chain", unsigned(st.cascades.size()));
    }
    // THE ISOTROPIC SINGLE VOLUME (Low): the reader's other half.
    {
        ...
        measureChain(e, scene, "isotropic volume", 1u);
    }
