...
iris::MeshNodePtr createNode(Database *db, Project *project)
{
    ...
    // shader quad that writes its plane's own depth with a relative bias toward
    // the camera — GRID-2; before that a 1 cm lift, two orders of magnitude
    // clear of this) — and measured the cost of doing so first: moving the ground
