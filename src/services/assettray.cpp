...
/// never added keeps its own tile.
...
bool isNodeOwnRow(Database *db, const AssetRecord &record)
{
    ...
    if (isType(record, ModelTypes::ParticleSystem)) {
        const QByteArray definition = db->fetchAssetData(record.guid).trimmed();
...
bool foldedIntoCompanion(Database *db, const QString &projectGuid, const QString &textureGuid)
{
    QStringList companions;
    for (const QString &companion : ImageMaterial::companionMaterials(textureGuid)) {
        if (db->isAssetPinnedBy(projectGuid, companion)
            || db->fetchAsset(companion).projectGuid == projectGuid)
            companions << companion;
    }
    if (companions.isEmpty()) return false;
    for (const QString &depender : db->fetchDependers(textureGuid, projectGuid))
...
QStringList hidden(Database *db, const QString &projectGuid, const QVector<AssetRecord> &records)
{
    QStringList out;
    if (!db) return out;
    const QSet<QString> claimed = avatarClosure(db, projectAvatars(db, projectGuid));
    for (const AssetRecord &record : records) {
        ...
        if (!record.parent.isEmpty() && record.parent != projectGuid
            && !db->fetchAsset(record.parent).guid.isEmpty()) {
        ...
        if (isNodeOwnRow(db, record)) { out.append(record.guid); continue; }
        // 5. an image whose tile is its companion material.
        if (isType(record, ModelTypes::Texture) && foldedIntoCompanion(db, projectGuid, record.guid))
            out.append(record.guid);
    }
    return out;
}
...
{
    const QStringList drop = hidden(db, projectGuid, records);
                          ...
                          int typeFilter)
{
    ...
    if (folder == projectGuid) {
        for (const auto &pinned : db->fetchProjectPinnedAssets(projectGuid)) {
            if (typeFilter > 0 && pinned.type != typeFilter) continue;
