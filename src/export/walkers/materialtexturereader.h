...
// (slot uniform name -> Texture2D), and also declares them as Texture-typed
// property rows. Exporters that need "every texture this material uses"
// (manifests, dependency gathering, inventory) enumerate through here;
// exporters with per-slot semantics (glTF's channel packing) read single slots
// via textureSlotSource.
...
struct TextureSlot
{
    QString role;     // the document slot name ("u_baseColorMap", …) or the
                      // property row name ("baseColorMap", …)
...
/// order), then any Texture-typed property rows not already seen.
