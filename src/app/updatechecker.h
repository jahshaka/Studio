...
public:
	...
	/// it is gone. Qt >= 6.7 applies a 30 s default to a plain
	/// QNetworkAccessManager, so without this line a launch behind a
	/// black-holing firewall keeps a socket, a reply and a timer alive for
	/// half a minute past startup before finishing empty — and the number
	/// would be Qt's, changeable under us, rather than one this check chose.
