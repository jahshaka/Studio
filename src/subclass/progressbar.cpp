...
void ProgressBar::configureUI()
{
	...
	connect(closeBtn, &QPushButton::clicked, [=]() {
		...
		connect(cancel, &QPushButton::clicked, [=]() { widget->hide(); });
	...
	confirm->setText("Yes");
	confirm->setStyleSheet("QPushButton{color: rgba(255, 255, 255, 1); background:rgba(230,230,230,.0); border: 1px solid rgba(0,0,0,.5); padding: 5px;}"
...
void ProgressBar::showConfirmationDialog()
{
	...
	widget = new QWidget;
	widget->setFixedHeight(confirmationSize - regularHeight);
