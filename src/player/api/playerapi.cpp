...
QVector<VerbInfo> PlayerApi::verbs() const
{
    return {
        ...
        { "screenshot", "player.screenshot(path, {width?, height?, probes?, grade?, postFx?}) -> {path, width, height, grade, encoding, center:{r,g,b}, probes:[...]}",
          ...
          "things. Works before the Player page has ever been shown. Where the screen-probe gather runs the shot waits for world.giStatus().giAtRest first, exactly as editor.screenshot does (at most 2000 frames, the player's clock frozen): a still scene photographs the held rest picture, byte-stable; a scene with something moving every frame photographs the gather's settled history, which is not. `probes` are "
