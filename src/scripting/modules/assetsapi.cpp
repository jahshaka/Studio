...
QVector<VerbInfo> AssetsApi::verbs() const
{
    return {
        ...
        { "storeStatus", "assets.storeStatus() -> {root, online, missing, deviceWaits}",
          "Store reachability: the active root, whether it is reachable (offline mode keeps the catalog fully usable), and how many library rows have no folder under it. `deviceWaits` is how many times THIS PROCESS has waited for the storage device — one per file the store actually fsynced (a hardlinked object writes no bytes and waits for nothing) — counted where the wait happens, in AssetCas::flushStaged. It is monotonic and process-wide, so only differences mean anything; bracket a gesture with two reads to assert that it cost no durable write, which is how \"no fsync on the thread that draws\" (FSYNC-2) stops being a claim and becomes a test.",
          Needs::Document },
