...
QVector<VerbInfo> NodeApi::verbs() const
{
    return {
        ...
        { "mobility", "node.mobility(id) -> {setting, resolved, reason, graphStatic}",
          ...
          "THE SETTING IS SAVED WITH THE SCENE, and only the setting: \"auto\" writes nothing "
          "and is re-derived on load, so a document always classifies under today's rule rather "
          "than the rule of the build that saved it, while a human's \"static\" or \"movable\" "
          "is carried in the file and beats the rule. The write is undoable, like every other "
          "reflected row.\n\n"
