...
QVector<VerbInfo> WorldApi::verbs() const
{
    return {
        ...
        { "gi", "world.gi({tier, mode, quality, bounces, cascades, cascadeSet, cascadeInstanceCap, dragOnMoverChannel, cards, cardBudget, cardRadius, pccGrid, updateBudget, probeSize, probeHdr, probeShadows, overlap, snapDeviation, snapSidesMin, snapSidesMax, rayMarchStepScale, gather, ddgi, ddgiIntensity}) -> bool",
          ...
          "'ddgiIntensity' (0..64, default 1) scales that replacement. 1.0 is the field's own physical answer - the cosine integral of the same voxel chain and the same environment the pixel's cones read, in the same units - so it is the default and the value a physically lit scene wants; any other value is an artistic trim of the diffuse bounce, and 0 leaves the field bound while contributing nothing (the A/B measurement). world.giStatus()'s ifdBound / ifdProbes / ifdProbesPerFrame / ifdRefinesOwed / giAtRest report what the renderer did with all of this. "
    ...
    if (params.contains("ddgiIntensity")) {
        ...
        if (v < 0.0 || v > 64.0)
            return fail(QStringLiteral(
                ...
                "voxel-cone diffuse OFF (the field REPLACES it, it does not add to it). 1.0 "
                "is the field's own physical answer - the integral of the same voxel chain "
                "and environment the cones read, in the same units - and the default; any "
                "other value is an artistic trim of the diffuse bounce, and 0 leaves the "
                "field bound and contributing nothing, which is the A/B measurement."));
