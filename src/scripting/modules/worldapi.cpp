...
QVector<VerbInfo> WorldApi::verbs() const
{
    return {
        ...
        { "setAtomDraw", "world.setAtomDraw(on) -> bool",
          ...
          Needs::Document },
        { "setAtomView", "world.setAtomView('off'|'triangles'|'levels'|'buckets'|'objects') -> bool",
          "THE ATOM VIEW: a real-time false-colour picture of what the visibility buffer holds, painted over the finished frame of every view of the open scene that draws through it (the viewport, and the editor's screenshots while it is on) wherever an object drawn by the id pass is the visible surface. "
...
QVariantMap WorldApi::giStatus()
{
    ...
    if (!st.available)
        return QVariantMap{ { QStringLiteral("mode"), requested },
                            ...
                            { QStringLiteral("ifdScrolls"), 0.0 },
                            { QStringLiteral("ifdReplacements"), 0.0 },
                            ...
                            { QStringLiteral("chainInjectionRefusals"), 0.0 },
                            { QStringLiteral("chainInjectionsPeakFrame"), 0 },
    ...
    return QVariantMap{ { QStringLiteral("mode"), st.mode },
                        ...
                        { QStringLiteral("ifdScrolls"), double(st.ifdScrolls) },
                        { QStringLiteral("ifdReplacements"), double(st.ifdReplacements) },
                        ...
                        { QStringLiteral("chainInjectionRefusals"), st.chainInjectionRefusals },
                        { QStringLiteral("chainInjectionsPeakFrame"), st.chainInjectionsPeakFrame },
                        ...
                        { QStringLiteral("gather"),
                          QVariantMap{
                              ...
                              { QStringLiteral("stereo"), st.gather.stereo },
                              { QStringLiteral("eyeProbesX"), st.gather.eyeProbesX },
                              { QStringLiteral("vramBytes"), st.gather.vramBytes },
                              { QStringLiteral("historyBytes"), st.gather.historyBytes },
                              { QStringLiteral("placeMs"), double(st.gather.placeMs) },
                              { QStringLiteral("traceMs"), double(st.gather.traceMs) },
                              { QStringLiteral("filterMs"), double(st.gather.filterMs) },
...
QVariantMap WorldApi::atomStatus()
{
    ...
    return QVariantMap{ { QStringLiteral("live"), st.live },
                        ...
                        { QStringLiteral("view"), atomView() },
                        { QStringLiteral("viewPaintable"), st.viewPaintable },
                        { QStringLiteral("cutValid"), st.cutValid },
                        { QStringLiteral("cutClusters"), st.cutClusters },
                        { QStringLiteral("cutTriangles"), double(st.cutTriangles) },
                        { QStringLiteral("cutIndices"), st.cutIndices },
                        { QStringLiteral("cutEvaluated"), st.cutEvaluated },
                        { QStringLiteral("cutIndexBudget"), st.cutIndexBudget },
                        { QStringLiteral("cutOverflow"), st.cutOverflow },
                        { QStringLiteral("cutMissing"), st.cutMissing },
                        { QStringLiteral("cutOverflowIndices"), st.cutOverflowIndices } };
}
...
namespace {
const char *const kAtomViewNames[] = { "off", "triangles", "levels", "buckets", "objects" };
}

bool WorldApi::setAtomView(const QString &view)
{
    auto scene = sceneOrFail(QStringLiteral("world.setAtomView"));
    if (!scene) return false;
    int mode = -1;
    for (int i = 0; i < 5; ++i)
        if (view.compare(QLatin1String(kAtomViewNames[i]), Qt::CaseInsensitive) == 0) mode = i;
    if (mode < 0) {
        fail(QStringLiteral("world.setAtomView: unknown view '%1' (off, triangles, levels, buckets, objects)")
                 .arg(view));
        return false;
    }
    if (!host.isEngineReady() || !host.viewport) return false;
    jahshaka::engine::Scene *es = host.viewport->engineScene();
    if (!es) return false;
    if (mode != 0 && !es->atomViewPaintable()) {
        fail(QStringLiteral("world.setAtomView: the viewport carries no id pass, so the '%1' view "
                            "would paint nothing — the Low tier's viewport draws straight into its "
                            "window, and world.setAtomDraw(false) shuts the split "
                            "(world.atomStatus().viewPaintable)")
                 .arg(view));
        return false;
    }
...
QString WorldApi::atomView()
{
    if (!host.isEngineReady() || !host.viewport) return QStringLiteral("off");
    jahshaka::engine::Scene *es = host.viewport->engineScene();
    const int mode = es ? int(es->atomView()) : 0;
    return QLatin1String(kAtomViewNames[(mode >= 0 && mode < 5) ? mode : 0]);
}
...
QVariantMap WorldApi::tierTable()
{
    ...
    for (const QString &name : worldmodes::photonTierNames()) {
        ...
        photonList.append(QVariantMap{
            ...
            // Epic tier), and its VR column (worldmodes::photonVrGather, PHOTON-GA-VR).
            { QStringLiteral("gather"), gatherOf(worldmodes::photonGather(t)) },
            { QStringLiteral("vrGather"), gatherOf(worldmodes::photonVrGather(t)) },
