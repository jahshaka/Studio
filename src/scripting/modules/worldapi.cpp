        ...
        auto chainOf = [&](const jahshaka::engine::GiQualityFacts &f) {
            ...
            for (int i = 0; i < f.cascadeCount; ++i)
                rows.append(QVariantMap{
                    { QStringLiteral("halfSize"), double(f.cascades[i].halfSize) },
                    { QStringLiteral("resolution"), f.cascades[i].resolution },
                    { QStringLiteral("stepCells"), double(f.cascades[i].stepCells) },
                    { QStringLiteral("cell"),
                      double(jahshaka::engine::giCascadeCell(f.cascades[i])) } });
            return rows;
        };
