...
QVector<VerbInfo> WorldApi::verbs() const
{
    return {
        ...
        { "gi", "world.gi({tier, mode, quality, bounces, cascades, cascadeSet, cascadeInstanceCap, dragOnMoverChannel, pccGrid, updateBudget, probeSize, probeHdr, probeShadows, overlap, snapDeviation, snapSidesMin, snapSidesMax, rayMarchStepScale, ddgi, ddgiIntensity, ddgiAmbient}) -> bool",
          ...
          "'dragOnMoverChannel' (MOVER-1, default FALSE = today's behaviour) says what happens to "
          "the room's stored lighting WHILE an object is being dragged. Today a Still object that is "
          "being dragged stales the cascade grid on every frame it moves and the scheduler answers "
          "with one cascade re-voxelisation per frame (~1.4 ms of CPU submission on the Mirror Room), "
          "and the frames in between are read through a volume that holds the object in two places at "
          "once. With this true, the SECOND move inside the settle window promotes the object onto the "
