...
QString giSourceToJs(int v)
{
    return v == 0 ? QStringLiteral("voxel")
         : (v == 1 ? QStringLiteral("raster") : QStringLiteral("auto"));
}
...
QVector<VerbInfo> WorldApi::verbs() const
{
    return {
        ...
        { "shadows", "world.shadows({enabled, mapBudget}) -> bool",
          ...
          Needs::Document },
        { "gi", "world.gi({tier, mode, quality, bounces, cascades, cascadeSet, cascadeInstanceCap, pccGrid, updateBudget, probeSize, probeHdr, probeShadows, overlap, snapDeviation, snapSidesMin, snapSidesMax, rayMarchStepScale, ddgi, ddgiIntensity, ddgiAmbient, ddgiSource}) -> bool",
...
bool WorldApi::gi(const QVariantMap &params)
{
    ...
    if (params.contains("ddgiSource")) {
        const QString v = params.value("ddgiSource").toString().trimmed().toLower();
        int wanted = -1;
        if (v == QLatin1String("auto")) wanted = -1;
        else if (v == QLatin1String("voxel")) wanted = 0;
        else if (v == QLatin1String("raster")) wanted = 1;
        else
            return fail(QStringLiteral(
                "world.gi: ddgiSource must be \"auto\", \"voxel\" or \"raster\" — where the "
                "irradiance field's probes get their light. voxel cone-traces the voxel volume "
                "(cheap; blind to skinned/animated geometry, which stays a static voxel of "
                "itself), raster renders six 32x32 faces per probe from the live scene under the "
                "same updateBudget (sees rigs move). auto is the tier's choice, voxel at every "
                "tier, epic included; raster is an Advanced opt-in."));
        scene->giDdgiSource = wanted;
    }
...
QVariantMap WorldApi::giStatus()
{
    ...
    if (!st.available)
        return QVariantMap{ { QStringLiteral("mode"), requested },
                            ...
                            { QStringLiteral("ifdSource"), QStringLiteral("voxel") },
    ...
    return QVariantMap{ { QStringLiteral("mode"), st.mode },
                        ...
                        { QStringLiteral("ifdSource"),
                          st.ifdRaster ? QStringLiteral("raster") : QStringLiteral("voxel") },
