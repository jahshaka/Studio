...
QVector<VerbInfo> WorldApi::verbs() const
{
    return {
        ...
        { "modeTable", "world.modeTable() -> object",
          "The World Mode registry itself: every row's id, label, group, type, options, per-tier values, cost note and availability. This is what the World panel and the docs are generated from. The cost notes and option names are THIS MACHINE'S: a row that explains reflections at High and Epic, and the technique's name ('VCT + rays' against 'VCT + probes'), read differently where the scene traces rays, because no probe grid is built there.",
          Needs::Document },
...
QVariantMap WorldApi::modeTable()
{
    ...
    // The texts are the machine's (worldmodes::rowCost): with no engine the
    // answer is the no-rays one, exactly what the World panel shows then.
    ...
    for (const worldmodes::Row &r : worldmodes::rows()) {
        ...
        for (const worldmodes::EnumOption &o : r.options)
            options.append(QVariantMap{ { "id", o.id },
                                        { "label", worldmodes::optionLabel(r, o, rays) },
                                        { "value", o.value } });
