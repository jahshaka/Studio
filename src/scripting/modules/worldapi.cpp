...
QVector<VerbInfo> WorldApi::verbs() const
{
    return {
        ...
        { "shadows", "world.shadows({enabled, mapBudget}) -> bool",
          ...
          Needs::Document },
        { "gi", "world.gi({tier, mode, quality, bounces, dynamicProbes, light, boundsMin, boundsMax, pccGrid, updateBudget, probeHdr, probeShadows, overlap, snapDeviation, snapSidesMin, snapSidesMax, rayMarchStepScale, ddgi, ddgiIntensity, ddgiAmbient, ddgiSource}) -> bool",
          ...
          "An unknown key is REFUSED with the list of the ones that exist.",
          Needs::Document },
        { "giStatus", "world.giStatus() -> {mode, requestedMode, probeCount, pccBound, vctBound, boundsMin, boundsMax, probeRegionMin, probeRegionMax, probeShapeMin, probeShapeMax, probeHdr, probeShadows, probeUpdatesPerFrame, dynamicProbes, dynamicProbeUpdates, cubemapProbeSlotsPerCell, probesClampedToRegion, worstProbeShapeCellRatio, reusedLastRefresh, ifdBound, ifdProbes, ifdConverged, ifdProbesPerFrame, ifdSource, live}",
