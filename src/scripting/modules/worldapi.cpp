...
QVector<VerbInfo> WorldApi::verbs() const
{
    return {
        ...
        { "giStatus", "world.giStatus() -> {mode, requestedMode, probeCount, pccBound, probeGridByRays, probePlacements, probeCapturesTotal, vctBound, boundsMin, boundsMax, voxelMetres, probeRegionMin, probeRegionMax, probeShapeMin, probeShapeMax, probeCaptureSize, probesDropped, probeHdr, probeShadows, probeUpdatesPerFrame, cubemapProbeSlotsPerCell, probesClampedToRegion, probesExceedingCell, probeGateCrossings, worstProbeShapeCellRatio, reusedLastRefresh, ifdBound, ifdProbes, ifdProbesPerFrame, ifdTargetSamples, ifdRefinesOwed, giAtRest, ifdMin, ifdMax, ifdFollows, probeCapturesLastFrame, probeCapturesDeferred, staleProbes, lastStaleReason, staleSerial, rebuilds, movableItems, movableLights, mobilityMisses, lastMobilityMiss, mobilityRebuilds, cards, rayQuery, gather, live}",
          ...
          Needs::Document },
        { "atomStatus", "world.atomStatus() -> {live, atomItems, pbsItems, notPbs, customPiece, blended, alphaTested, skinned, multiSubmesh, noRow, stockItems, materials, buckets, twins, decodeDraws}",
          "THE VISIBILITY BUFFER'S SPLIT, as the renderer decides it for the open scene (ATOM S3-DRAW). 'atomItems' is how many shown objects the ID PASS draws and the material DECODE shades: an object with a row in the GPU scene, an opaque PBR material the decode can serve and one submesh. 'pbsItems' is how many stay on the stock PBR shader, and the next seven fields split them by the FIRST reason that holds, in this order: 'notPbs' (an unlit or other non-PBR material), 'customPiece' (a material carrying its own shader piece), 'blended' (a transparent, faded or refractive material), 'alphaTested' (a cut-out), 'skinned' (a posed character: the id pass reads the mesh's stored bind pose), 'multiSubmesh' (a mesh with more than one submesh) and 'noRow' (no readable geometry row for its finest level). 'stockItems' counts objects in a queue the split never touches (gizmos, wires, the sun disc, distortion). 'materials' is how many distinct PBR materials the atom objects wear and 'buckets' how many DECODE DRAWS they need: materials that share a shader permutation, a texture set and a material pool are shaded by ONE full-screen draw, so buckets is the decode's cost and materials is not. 'twins' is how many decode twins the renderer holds and 'decodeDraws' this scene's decode draws. Hidden objects are in no count. 'live' is false without an engine viewport or GPU scene, and every other field is then 0.",
          Needs::Document },
...
QVariantMap WorldApi::atomStatus()
{
    ...
    return QVariantMap{ { QStringLiteral("live"), st.live },
                        ...
                        { QStringLiteral("multiSubmesh"), st.multiSubmesh },
