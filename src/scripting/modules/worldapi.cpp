...
QVariantMap WorldApi::tierTable()
{
    ...
    for (const QString &name : worldmodes::photonTierNames()) {
        ...
        QVariantList chain;
        if (worldmodes::photonCascades(t))
            for (int i = 0; i < facts.cascadeCount; ++i)
                chain.append(QVariantMap{
                    { QStringLiteral("halfSize"), double(facts.cascades[i].halfSize) },
                    { QStringLiteral("resolution"), facts.cascades[i].resolution },
                    { QStringLiteral("cell"),
                      double(jahshaka::engine::giCascadeCell(facts.cascades[i])) } });
