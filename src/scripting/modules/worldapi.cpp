...
QVector<VerbInfo> WorldApi::verbs() const
{
    return {
        ...
        { "setAtomView", "world.setAtomView('off'|'triangles'|'levels'|'buckets'|'objects') -> bool",
          ...
          "'triangles' colours every triangle a random bright colour (per object, level and triangle); 'levels' shows the level of detail the renderer chose, a fixed ramp from level 0 = red, 1 = orange, 2 = yellow, 3 = lime, 4 = green, 5 = cyan, 6 = blue to 7 = violet (the coarsest); 'buckets' gives each material DECODE DRAW its own colour (world.atomStatus().buckets counts them; materials that share one draw share a colour); 'objects' gives each object its own colour; 'off' is the ordinary picture, byte for byte. "
