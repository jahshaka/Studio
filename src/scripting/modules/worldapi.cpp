...
/// The probe source in world.gi's spelling: -1 auto, 0 voxel, 1 raster.
...
/// THE GI MODE NAMES, ONCE. Four private copies of this table lived in this
/// file (audit A F2) and every one of them was positional, so deleting Instant
/// Radiosity from the enum would have re-labelled every mode by one without a
/// single compiler error. One table, in enum order, `GiMode::VctPccHybrid` last.
const char *const kGiModeNames[] = { "off", "vct", "vct_pcc_hybrid" };

/// The world.gi fields that are NOT quality-registry rows (those ride the
/// registry snapshot: giMode, giQuality, giBounces, giDdgi).
...
QVector<VerbInfo> WorldApi::verbs() const
{
    return {
        ...
        { "fog", "world.fog({enabled, color, density, heightDensity, heightFalloff, heightLevel, breakMinBrightness, breakFalloff, atmosphere}) -> bool",
          ...
          "distance where the linear and the exponential curve are both half fogged. "
        ...
        { "shadows", "world.shadows({enabled, mapBudget}) -> bool",
          ...
          Needs::Document },
        { "gi", "world.gi({tier, mode, quality, bounces, cascades, cascadeSet, cascadeInstanceCap, dragOnMoverChannel, cards, cardBudget, cardRadius, pccGrid, updateBudget, probeSize, probeHdr, probeShadows, overlap, snapDeviation, snapSidesMin, snapSidesMax, rayMarchStepScale, gather, ddgi, ddgiIntensity, ddgiAmbient}) -> bool",
          ...
          "for the length of the gesture; off is available per project) says what happens to "
          "the room's stored lighting WHILE an object is being dragged. With it OFF, a Still object "
          "that is being dragged stales the cascade grid on every frame it moves and the scheduler "
          "answers with one cascade re-voxelisation per frame (~1.4 ms of CPU submission on the Mirror "
          "Room), and the frames in between are read through a volume that holds the object in two "
          "places at once. With it on, the SECOND move inside the settle window promotes the object onto the "
          "MOVER CHANNEL the Mobility design already defines: it carries the movable channel and no GI "
          "geometry bit, so it stops bouncing light into the room and the room stops storing its "
          "shadow, and it is lit by the irradiance field and the voxel cones at its live pose like "
          "every Movable object in the scene. The cascades then pay ONE re-voxelisation at the "
          "promotion — which clears the copy of it they hold at its old pose — and one more when the "
          "gesture ends, instead of one per frame of it. THE PICTURE AT REST IS IDENTICAL either way "
        ...
        { "giVoxelStats", "world.giVoxelStats({cascade}) -> {available, cascade, width, height, depth, format, formatMax, multiplier, peak, peakDirect, meanLit, voxels, voxelsLit, voxelsAtMax, voxelsAboveOne, directAtMax, emissiveFormat, peakEmissive, emissiveAtMax, emissiveAboveOne}",
          ...
          "It exists because the voxel volume is the one place in this renderer where a PHYSICAL quantity is kept in a FIXED-RANGE format, and whether it FITS cannot be seen in the picture: a clipped voxel draws a picture that is merely dimmer, with no hitch and no log line, exactly like a scene with less bounce in it. That mechanism cost a measurement once already (PHOTON-M2 read a saturated gather as a contracting bounce series). "
          "EVERY NUMBER IS IN THE STORE'S OWN NORMALISED UNITS, which are the surface's outgoing radiance times 'multiplier' (the renderer normalises by the brightest lamp so the DIRECT term lands at or below the store's ceiling; divide by 'multiplier' for scene radiance). 'format' is the total volume's pixel format and 'formatMax' the largest value it can hold in those units — 1.0 for the 8-bit stores, 0 for a float one, which has no ceiling worth naming. "
          "'peak' is the brightest channel anywhere in the TOTAL volume (direct light plus every bounce) and 'peakDirect' the same over the DIRECT volume the bounce iterates from — that one is at or below the ceiling BY CONSTRUCTION, so a 'peakDirect' above it is a defect in the normalisation itself rather than a bright scene. 'meanLit' is the mean channel maximum over the voxels that hold any light, which is what a convergence test watches stop moving. "
          ...
          "'available' false — and nothing else — when there is no engine viewport, no voxel arm on this scene, or no such cascade; there is no document-side answer to fall back on, so a zero here is never a measurement.",
          Needs::Document },
        { "photon", "world.photon({enabled, tier}) -> {enabled, tier, custom, deviations, technique, quality, ddgi, bounces, row, ddgiIntensity, ddgiAmbient, updateBudget}",
        ...
        { "shadowStatus", "world.shadowStatus() -> {live, resolution, maps, pssmSplits, focusedMaps, casters, budget, requestedBudget, atlasWidth, atlasHeight, atlasBytes, reflectAtlasBytes, probeAtlasBytes, mapped:[{slot, node, cached, dirty, pssm, passes}], unmapped:[guid], countersMeasured, shadowPassesLastFrame, cachedMapRendersLastFrame, reflectPassesLastFrame, probePassesLastFrame, reflectLampPassesLastFrame, probeLampPassesLastFrame, cachedInstances, uncachedInstances, viewCached, mapsDirtiedLastFrame, atlasRebuilds, casterWalkItems, shaderLightMismatches, sun, secondaryDirectionals}",
          ...
          "render passes prepared while the renderer's shadow node declared fewer shadow maps than its own "
        ...
        { "ssrMarch", "world.ssrMarch([\"checker\"|\"refined\"|\"dither\"]) -> \"checker\"|\"refined\"|\"dither\"",
          ...
          "\"refined\" is the DEFAULT (owner, 2026-09-20, on PICTURES-1's sheet): it makes a crossing a "
          "SIGN CHANGE — the ray was in front at the previous sample and is behind at this one, which is "
          "a crossing whatever the phase — and then asks the thickness question at the BISECTED crossing, "
          "within 1/32 of a step of the true one. The step stops deciding WHETHER a ray hit and only "
          "decides how finely the ray is sampled, so a curved glossy surface shows ONE solid reflection "
          "instead of the nested arcs, for +0.67 ms at 4K and nothing measurable below it. "
          "\"checker\" is the march exactly as it shipped before that call, and stays selectable for a "
          "project that wants the older picture back. "
          "\"dither\" is the MINIMAL candidate: checker's hit rule with a 4x4 ordered phase in place of "
          "the two-value checkerboard the march has always used. It changes what a ray SAMPLES and nothing "
          "about what counts as a hit, so the reflection covers exactly what checker covers and only the "
          ...
          "with no argument it reads. One undo step. There is still no World panel row: the choice was "
          "made on the pictures and the default is the answer for everybody.",
          Needs::Document },
        { "vr", "world.vr({flySpeed?, fly?, turn?, snapTurnDegrees?, smoothTurnDegreesPerSecond?, dominant?, hands?}) -> object",
          ...
          "`flySpeed` is METRES PER SECOND (default 15; the stick's boost multiplies it, and "
          "so does the person's own speed dial editor.cameraSpeed — this is the project's "
          "BASE, and vr.state() reports the effective number), "
...
bool WorldApi::gi(const QVariantMap &params)
{
    ...
    static const QStringList known = {
        ...
        // an object is dragged, so the owner picks it from the pictures and the
        // World panel gets a row only after that.
        QStringLiteral("dragOnMoverChannel"),
        // THE SURFACE CACHE (SURFACE-CACHE phase 2), verb-only and OFF by
        // default: nothing READS a card until phase 4, so a World panel row
        // would be a dial with no picture behind it. A suite, a measurement and
        // the render monitor turn it on.
        // THE SCREEN-PROBE GATHER's row. Without this line `refuseUnknownKeys`
        // below refuses the key and the handler that reads it is unreachable —
        // the lane's one user door, dead (the lead's read).
        QStringLiteral("gather"),
        QStringLiteral("cards"), QStringLiteral("cardBudget"),
        QStringLiteral("cardRadius")
    };
    ...
    // BY NAME rather than swallowed by the unknown-key list: a script that
    // still passes them is asking for a volume of its own, and this renderer no
    // longer has one to give. Same reasoning as world.sky's retired sun keys.
    for (const char *gone : { "boundsMin", "boundsMax", "autoBoundsMax" }) {
        if (!params.contains(QLatin1String(gone))) continue;
        return fail(QStringLiteral(
            "world.gi: '%1' is gone — THE LIT VOLUME IS AUTOMATIC NOW. The renderer fits it to "
            "the scene's content on every solve and there is nothing to pin or to cap. Read what "
            "it decided with world.giStatus().boundsMin / .boundsMax (and .voxelMetres), and keep "
            "one object out of the fit with node.setProperty(id, \"giBoundsExcluded\", true).")
                        .arg(QLatin1String(gone)));
    }

    const QString refusal = refuseUnknownKeys(
        QStringLiteral("world.gi"), params, known,
        QStringLiteral("'autoRefresh' was RETIRED by the update-budget model: it is "
                       "'updateBudget' now (0 = paused, which is the old autoRefresh false; "
    ...
    if (params.contains("mode")) {
        ...
        else if (m == "instant_radiosity")
            // DELETED, AND SAID SO (PHOTON_SPEC E2 (4)). A refusal by name beats
            // a silent remap here: a script that asks for it is asking for a
            // technique with different physics (one light, no emissives, a CPU
            // re-trace per light move), and the honest answer is what replaced it.
            return fail(QStringLiteral(
                "world.gi: 'instant_radiosity' was deleted — Photon's Low tier is two "
                "camera-centred voxel cascades with the irradiance field on. Use 'vct'."));
        else return fail(QStringLiteral("world.gi: unknown mode '%1' (off, vct, vct_pcc_hybrid)").arg(m));
    ...
    if (params.contains(QStringLiteral("cardBudget"))) {
        ...
        if (v < 0)
            return fail(QStringLiteral(
                "world.gi: cardBudget is 0 or more TEXELS a frame (0 = the quality tier's "
                "own). It is the card capture's per-frame budget — texels rather than cards "
                "because a 16-texel card and a 128-texel card are not the same work, and "
                "texels rather than milliseconds because a wall clock measures nothing here."));
        scene->giCardBudgetTexels = v;
    }
    if (params.contains(QStringLiteral("cardRadius"))) {
        const double v = params.value(QStringLiteral("cardRadius")).toDouble();
        if (v < 0.0)
            return fail(QStringLiteral(
                "world.gi: cardRadius is 0 or more METRES (0 = the quality tier's own) — an "
                "instance further than this from the camera holds no card pages at all."));
        scene->giCardRadius = float(v);
    }
    // A DRAGGED STILL ON THE MOVER CHANNEL (MOVER-1). A plain bool: it is not
    // a tier question (a tier says how much a solve may cost; this says what a
    // gesture does to the room's stored lighting) and it is not continuous.
    if (params.contains(QStringLiteral("dragOnMoverChannel"))) {
        const QVariant v = params.value(QStringLiteral("dragOnMoverChannel"));
        if (v.typeId() != QMetaType::Bool)
            return fail(QStringLiteral(
                "world.gi: dragOnMoverChannel is true or false — whether an object being "
                "DRAGGED rides the mover channel for the length of the gesture (it stops "
                "bouncing light into the room and is lit by the field and the cones at its "
                "live pose) instead of being re-voxelised on every frame it moves"));
        scene->giDragMoverChannel = v.toBool() ? 1 : 0;
    }
    if (params.contains("cascadeSet")) {
        const QVariantList rows = params.value("cascadeSet").toList();
        if (rows.size() > 8)
            return fail(QStringLiteral("world.gi: cascadeSet takes at most 8 cascades, got %1")
                            .arg(rows.size()));
        QVector<iris::Vec3> set;
        for (const QVariant &r : rows) {
            const QVariantMap row = r.toMap();
            const double half = row.value(QStringLiteral("halfSize")).toDouble();
            const double res  = row.value(QStringLiteral("resolution")).toDouble();
            const double step = row.value(QStringLiteral("stepCells"), 0.0).toDouble();
            if (half <= 0.0 || res < 16.0 || res > 256.0)
                return fail(QStringLiteral(
                    "world.gi: each cascadeSet row needs a positive halfSize (metres, the "
                    "HALF-extent of the cube this cascade covers) and a resolution in 16..256; "
                    "stepCells is optional (0 = derive it, which keeps every cascade stepping "
                    "the same distance). Got halfSize %1, resolution %2.")
                                .arg(half).arg(res));
            // A TABLE THAT DOES NOT GROW OUTWARD CANNOT BE HONOURED AT ALL, and
            // a request the renderer cannot honour is not honoured halfway
            // (the same rule as the rows above). Ogre chains a cascade over the
            // coarser one behind it and derives the hand-over LOD from the
            // ratio of their cells, so an outer cascade that is smaller — or
            // finer — than the one before it makes that ratio <= 1 and the
            // march never hands over: the far bounce is simply missing.
            if (!set.isEmpty()) {
                const float prevHalf = set.last().x(), prevRes = set.last().y();
                if (float(half) <= prevHalf ||
                    float(half) * 2.0f / float(res) <= prevHalf * 2.0f / prevRes)
                    return fail(QStringLiteral(
                        "world.gi: cascadeSet must grow OUTWARD — every row needs a bigger "
                        "halfSize AND a bigger cell (halfSize*2/resolution) than the one "
                        "before it, because each cascade hands the ray march over to the "
                        "coarser one behind it. Row %1 asks for halfSize %2 at resolution %3 "
                        "(cell %4 m) after halfSize %5 (cell %6 m).")
                                    .arg(set.size() + 1).arg(half).arg(res)
                                    .arg(double(float(half) * 2.0f / float(res)))
                                    .arg(double(prevHalf))
                                    .arg(double(prevHalf * 2.0f / prevRes)));
            }
            set.append(iris::Vec3(float(half), float(res), float(step)));
        }
        scene->giCascadeSet = set;
    }
    ...
    if (params.contains("probeSize")) {
        const QVariant raw = params.value(QStringLiteral("probeSize"));
        bool numeric = false;
        const double d = raw.toDouble(&numeric);
        const int v = int(d);
        const bool power2 = v == 0 || (v >= 64 && v <= 1024 && (v & (v - 1)) == 0);
        if (!numeric || d != qFloor(d) || !power2)
            return fail(QStringLiteral(
                "world.gi: probeSize must be 0 or a power of two in 64..1024 — the pixel size "
                "of ONE reflection-probe cube face, got '%1'. A probe costs six faces at this "
                "size plus its mip chain, so the grid's video memory goes with the SQUARE of "
                "it: 256 (the default at Medium) is 4.0 MB per probe in HDR and 512 (the "
                "default at High and Epic) is 16.0 MB. 0 means follow the quality dial "
                "(128 at Low, 256 at Medium, 512 at High and Epic). "
                "world.giStatus().probeCaptureSize reports what the renderer resolved.")
                            .arg(raw.toString()));
    ...
    if (params.contains("light"))
        // The Instant Radiosity driving light. Refused by name, like the bounds
        // keys above it (PHOTON_SPEC E2 (4)): every light is a voxel light now.
        return fail(QStringLiteral(
            "world.gi: 'light' is gone with Instant Radiosity — every light in the scene "
            "is injected into the voxel volume, so there is no driving light to choose."));
...
QVariantMap WorldApi::photonRow(worldmodes::PhotonTier tier)
{
    ...
    return QVariantMap{
        ...
        { QStringLiteral("technique"),
          QString::fromLatin1(kGiModeNames[qBound(0, worldmodes::photonTechnique(tier), 2)]) },
...
/// The ray-query tier's reading as a plain map. Every field is what the
/// renderer HOLDS; `available` is the device's answer and `enabled` ours, so the
/// pair {true,false} is exactly "this GPU can trace and we told it not to" —
/// the no-rays switch, which is the picture a machine without ray tracing gets.
static QVariantMap rayQueryToJs(const IEditorViewport::GiStatusInfo::RayQueryInfo &rq)
{
    return QVariantMap{ { QStringLiteral("available"), rq.available },
                        { QStringLiteral("enabled"), rq.enabled },
                        { QStringLiteral("blasCount"), rq.blasCount },
                        { QStringLiteral("instances"), rq.instances },
                        { QStringLiteral("triangles"), rq.triangles },
                        { QStringLiteral("blasBytes"), double(rq.blasBytes) },
                        { QStringLiteral("tlasBytes"), double(rq.tlasBytes) },
                        { QStringLiteral("tlasMs"), double(rq.tlasMs) },
                        { QStringLiteral("blasMs"), double(rq.blasMs) },
                        { QStringLiteral("gatherMs"), double(rq.gatherMs) },
                        { QStringLiteral("lastWasRefit"), rq.lastWasRefit },
                        { QStringLiteral("tlasBuilds"), double(rq.tlasBuilds) },
                        { QStringLiteral("tlasRefits"), double(rq.tlasRefits) },
                        { QStringLiteral("blasBuilds"), double(rq.blasBuilds) },
                        // The reflect trace itself (R5): whether it ran, how many rays,
                        // what it cost — SMOKE-41 (2026-09-17) had to infer all three.
                        { QStringLiteral("reflect"), rq.reflect },
                        { QStringLiteral("reflectRays"), rq.reflectRays },
                        { QStringLiteral("reflectMs"), double(rq.reflectMs) } };
}

/// THE SURFACE CACHE, as a script reads it (SURFACE-CACHE phase 2). Every
/// counter is the model `gi.material_swap` set for the voxel side: a test drives
/// an edit and asserts what the renderer SPENT, not what it looks like.
static QVariantMap cardsToJs(const IEditorViewport::GiStatusInfo::CardsInfo &c)
{
    return QVariantMap{
        { QStringLiteral("built"), c.built },
        { QStringLiteral("pageSize"), c.pageSize },
        { QStringLiteral("pages"), c.pages },
        { QStringLiteral("pagesUsed"), c.pagesUsed },
        { QStringLiteral("bytesPerTexel"), c.bytesPerTexel },
        { QStringLiteral("bytes"), c.bytes },
        { QStringLiteral("emissiveFormat"), c.emissiveFormat },
        { QStringLiteral("instances"), c.instances },
        { QStringLiteral("cards"), c.cards },
        { QStringLiteral("radius"), c.radius },
        { QStringLiteral("queue"), c.queue },
        { QStringLiteral("budgetTexels"), c.budgetTexels },
        { QStringLiteral("capturesLastFrame"), c.capturesLastFrame },
        { QStringLiteral("texelsLastFrame"), c.texelsLastFrame },
...
QVariantMap WorldApi::giStatus()
{
    ...
    if (!scene) return QVariantMap();
    const QString requested = QString::fromLatin1(kGiModeNames[qBound(0, int(scene->giMode), 2)]);
    ...
    if (!st.available)
        return QVariantMap{ { QStringLiteral("mode"), requested },
                            ...
                            { QStringLiteral("boundsMin"), vecToJs(iris::Vec3()) },
                            { QStringLiteral("boundsMax"), vecToJs(iris::Vec3()) },
                            { QStringLiteral("voxelMetres"), 0.0 },
                            ...
                            { QStringLiteral("probesExceedingCell"), 0 },
                            ...
                            { QStringLiteral("ifdFollows"), 0.0 },
                            { QStringLiteral("probeCapturesLastFrame"), 0 },
                            { QStringLiteral("probeCapturesDeferred"), 0.0 },
                            { QStringLiteral("staleProbes"), 0 },
                            { QStringLiteral("lastStaleReason"), QStringLiteral("none") },
                            { QStringLiteral("rayQuery"),
                              rayQueryToJs(IEditorViewport::GiStatusInfo::RayQueryInfo()) },
                            { QStringLiteral("staleSerial"), 0.0 },
                            { QStringLiteral("rebuilds"), 0.0 },
                            { QStringLiteral("movableItems"), 0 },
                            { QStringLiteral("movableLights"), 0 },
                            { QStringLiteral("mobilityMisses"), 0.0 },
                            { QStringLiteral("lastMobilityMiss"), QString() },
                            { QStringLiteral("mobilityRebuilds"), 0.0 },
                            { QStringLiteral("cascades"), QVariantList() },
                            { QStringLiteral("cascadesAwaitingCamera"), false },
                            { QStringLiteral("awaitingVoxelTextures"), false },
                            // FALSE, not true: with no renderer nothing is
                            // running, and a reading that says a proxy is
                            // active in a headless run is a lie the `live`
                            // flag does not excuse (render audit I-10).
                            { QStringLiteral("cascadeVoxelLod"), false },
                            { QStringLiteral("cascadeProfileVr"), false },
                            { QStringLiteral("cascadeFullRebuilds"), 0.0 },
                            { QStringLiteral("cascadeDeferrals"), 0.0 },
                            { QStringLiteral("cascadeDirtyMajority"), 0.0 },
                            { QStringLiteral("chainSweeps"), 0 },
                            { QStringLiteral("chainSettles"), 0.0 },
                            { QStringLiteral("dragMovers"), 0 },
                            { QStringLiteral("dragMoverGestures"), 0.0 },
                            { QStringLiteral("cards"),
                              cardsToJs(IEditorViewport::GiStatusInfo::CardsInfo()) },
                            { QStringLiteral("live"), false } };
    QVariantList cascadeList;
    for (const auto &c : st.cascades) {
        QVariantList lodLevels;
        for (int n : c.lodLevels) lodLevels.append(n);
        cascadeList.append(QVariantMap{
            { QStringLiteral("halfSize"), double(c.halfSize) },
            { QStringLiteral("resolution"), c.resolution },
            { QStringLiteral("cell"), double(c.cell) },
            { QStringLiteral("step"), double(c.step) },
            { QStringLiteral("guaranteedRadius"), double(c.guaranteedRadius) },
            { QStringLiteral("centre"), vecToJs(iris::fromQt(c.centre)) },
            { QStringLiteral("rebuilds"), double(c.rebuilds) },
            { QStringLiteral("pending"), c.pending },
            { QStringLiteral("items"), c.items },
            { QStringLiteral("attached"), c.attached },
            { QStringLiteral("lastCpuMs"), double(c.lastCpuMs) },
            { QStringLiteral("lodLevels"), lodLevels },
            { QStringLiteral("voxelTriangles"), double(c.voxelTriangles) },
            { QStringLiteral("voxelDispatches"), double(c.voxelDispatches) } });
    }
    return QVariantMap{ { QStringLiteral("mode"), st.mode },
                        ...
                        // REPORTED AT LAST (render audit I-10): the engine has
                        // counted this since the 2026-09-07 fix wave and no
                        // caller could see it. Non-zero is a defect.
                        { QStringLiteral("probesExceedingCell"), st.probesExceedingCell },
                        ...
                        // rides cascade 0 and this box walks with the camera.
                        { QStringLiteral("ifdMin"), vecToJs(iris::Vec3(float(st.ifdMin.x()),
                                                                       float(st.ifdMin.y()),
                                                                       float(st.ifdMin.z()))) },
                        { QStringLiteral("ifdMax"), vecToJs(iris::Vec3(float(st.ifdMax.x()),
                                                                       float(st.ifdMax.y()),
                                                                       float(st.ifdMax.z()))) },
                        { QStringLiteral("ifdFollows"), double(st.ifdFollows) },
                        // THE PROBE CACHE (ENGINE_CACHE_POLICY_SPEC P1/P6/P7).
                        { QStringLiteral("probeCapturesLastFrame"), st.probeCapturesLastFrame },
                        { QStringLiteral("probeCapturesDeferred"), double(st.probeCapturesDeferred) },
                        { QStringLiteral("staleProbes"), st.staleProbes },
                        { QStringLiteral("lastStaleReason"), st.lastStaleReason },
                        { QStringLiteral("staleSerial"), double(st.staleSerial) },
                        { QStringLiteral("rebuilds"), double(st.rebuilds) },
                        // MOBILITY (REALTIME_REFLECTIONS_SPEC §3.3.4).
                        { QStringLiteral("movableItems"), st.movableItems },
                        { QStringLiteral("movableLights"), st.movableLights },
                        { QStringLiteral("mobilityMisses"), double(st.mobilityMisses) },
                        { QStringLiteral("lastMobilityMiss"), st.lastMobilityMiss },
                        { QStringLiteral("mobilityRebuilds"), double(st.mobilityRebuilds) },
                        // PHOTON (SPECS/PHOTON_SPEC.md P0): the cascade chain
                        // as BUILT, innermost first.
                        { QStringLiteral("cascades"), cascadeList },
                        { QStringLiteral("cascadesAwaitingCamera"), st.cascadesAwaitingCamera },
                        { QStringLiteral("awaitingVoxelTextures"), st.awaitingVoxelTextures },
                        { QStringLiteral("cascadeVoxelLod"), st.cascadeVoxelLod },
                        { QStringLiteral("cascadeProfileVr"), st.cascadeProfileVr },
                        { QStringLiteral("cascadeFullRebuilds"), double(st.cascadeFullRebuilds) },
                        { QStringLiteral("cascadeDeferrals"), double(st.cascadeDeferrals) },
                        { QStringLiteral("cascadeDirtyMajority"), double(st.cascadeDirtyMajority) },
                        { QStringLiteral("chainSweeps"), st.chainSweeps },
                        { QStringLiteral("chainSettles"), double(st.chainSettles) },
                        { QStringLiteral("dragMovers"), st.dragMovers },
                        { QStringLiteral("dragMoverGestures"), st.dragMoverGestures },
                        // THE SURFACE CACHE (SURFACE-CACHE phase 2).
                        { QStringLiteral("cards"), cardsToJs(st.cards) },
                        // THE RAY-QUERY TIER (SPECS/PHOTON_SPEC.md §7 R1).
                        { QStringLiteral("rayQuery"), rayQueryToJs(st.rayQuery) },
                        // THE SCREEN-PROBE GATHER (SCREEN_PROBE_GATHER_SPEC
                        // phase 1): the row resolved against the machine, and
                        // what the last gathered frame placed, traced and cost.
                        { QStringLiteral("gather"),
                          QVariantMap{
                              { QStringLiteral("on"), st.gather.on },
                              { QStringLiteral("running"), st.gather.running },
                              { QStringLiteral("stride"), st.gather.stride },
                              { QStringLiteral("octRes"), st.gather.octRes },
                              { QStringLiteral("raysPerProbe"), st.gather.raysPerProbe },
                              { QStringLiteral("probesX"), st.gather.probesX },
                              { QStringLiteral("probesY"), st.gather.probesY },
                              { QStringLiteral("probes"), st.gather.probes },
                              { QStringLiteral("adaptive"), st.gather.adaptive },
                              { QStringLiteral("adaptiveCap"), st.gather.adaptiveCap },
                              { QStringLiteral("adaptiveRequested"), st.gather.adaptiveRequested },
                              { QStringLiteral("raysPerFrame"), st.gather.raysPerFrame },
                              { QStringLiteral("targetW"), st.gather.targetW },
                              { QStringLiteral("targetH"), st.gather.targetH },
                              { QStringLiteral("atlasBytes"), st.gather.atlasBytes },
                              { QStringLiteral("placeMs"), double(st.gather.placeMs) },
...
{
    auto scene = sceneOrFail(QStringLiteral("world.giVoxelStats"));
    if (!scene) return QVariantMap();
    const int cascade = params.value(QStringLiteral("cascade"), 0).toInt();
    IEditorViewport::GiVoxelStatsInfo st;
    if (host.isEngineReady() && host.viewport) st = host.viewport->giVoxelStats(cascade);
    // No engine, no VCT arm, no such cascade: `available` false and nothing
    // else. A caller must never read a zero here as a measurement — this verb
    // has no document-side answer to fall back on, unlike world.giStatus().
    if (!st.available)
    ...
    return QVariantMap{ { QStringLiteral("available"), true },
                        { QStringLiteral("cascade"), st.cascade },
                        { QStringLiteral("width"), st.width },
                        { QStringLiteral("height"), st.height },
                        { QStringLiteral("depth"), st.depth },
                        { QStringLiteral("format"), st.format },
                        { QStringLiteral("formatMax"), double(st.formatMax) },
                        { QStringLiteral("multiplier"), double(st.multiplier) },
                        { QStringLiteral("peak"), double(st.peak) },
                        { QStringLiteral("peakDirect"), double(st.peakDirect) },
                        { QStringLiteral("meanLit"), st.meanLit },
                        { QStringLiteral("voxels"), double(st.voxels) },
                        { QStringLiteral("voxelsLit"), double(st.voxelsLit) },
                        { QStringLiteral("voxelsAtMax"), double(st.voxelsAtMax) },
                        { QStringLiteral("voxelsAboveOne"), double(st.voxelsAboveOne) },
                        { QStringLiteral("directAtMax"), double(st.directAtMax) },
                        { QStringLiteral("emissiveFormat"), st.emissiveFormat },
                        { QStringLiteral("peakEmissive"), double(st.peakEmissive) },
...
QVariantMap WorldApi::shadowStatus()
{
    ...
    if (!st.available) {
        ...
        out[QStringLiteral("reflectAtlasBytes")] = 0;
    ...
    out[QStringLiteral("reflectAtlasBytes")] = double(st.reflectAtlasBytes);
    out[QStringLiteral("probeAtlasBytes")] = double(st.probeAtlasBytes);
    QVariantList mapped;
    for (const IEditorViewport::ShadowMapEntry &e : st.mapped) {
        QVariantMap m;
        m[QStringLiteral("slot")] = e.slot;
        m[QStringLiteral("node")] = e.node;
        m[QStringLiteral("cached")] = e.isCached;
        m[QStringLiteral("dirty")] = e.dirty;
        m[QStringLiteral("pssm")] = e.pssm;
        m[QStringLiteral("passes")] = e.passesLastFrame;
        mapped.append(m);
    }
...
// iris::RayTracingMode for the reasoning. This verb is the whole authoring
// surface; the World panel's row calls exactly this document field through the
// same sceneprops key, and the renderer's half is Scene::rayTracingResolved().
QString WorldApi::rayTracing(const QString &mode)
{
    auto scene = sceneOrFail(QStringLiteral("world.rayTracing"));
    if (!scene) return QString();
    if (!mode.isEmpty()) {
        iris::RayTracingMode wanted = iris::RayTracingMode::Auto;
        if (!iris::rayTracingModeFromName(mode, wanted)) {
            // REFUSED, never guessed: a typo that silently meant "auto" would
            // be a scene that quietly stopped saying what it was authored for.
            fail(QStringLiteral("world.rayTracing: '%1' is not a state "
                                "(expected \"off\", \"auto\" or \"on\")").arg(mode));
            return QString();
        }
        WorldEdit edit(scene, { QStringLiteral("rayTracing") });
        sceneprops::set(scene, QStringLiteral("rayTracing"), int(wanted));
        edit.commit(host.services ? host.services->undo : nullptr,
...
QVariantMap WorldApi::get()
{
    ...
    out["gi"] = QVariantMap{ { "mode", kGiModeNames[qBound(0, int(scene->giMode), 2)] },
                             ...
                             // THE SURFACE CACHE's three rows, read back as
                             // authored (SURFACE-CACHE phase 2). `cards` is the
                             // off|auto|on word, not the resolved answer — what
                             // the renderer DID is world.giStatus().cards.
                             { "cards", giToggleToJs(scene->giCards) },
                             { "cardBudget", scene->giCardBudgetTexels },
                             { "cardRadius", double(scene->giCardRadius) },
...
QVariantMap WorldApi::postFx(const QVariantMap &params)
{
    ...
    // ...and the same for the METER'S percentile pair (EXPOSURE-2), for the
    // same reason: the renderer walks a cumulative weight and a reversed pair
...
// WHAT A TIER IS, FROM THE TABLES (render audit A5, lane CRUD-RENDER-1).
//
// The app half comes from worldmodes::kPhotonTable and the PHYSICAL half from
// the engine's `giQualityFacts` — the same two tables the tier tooltips are
// generated from, and the same ones OgreGi.cpp reads to build the chain. A
// script (or a doc, or a test) can therefore check the renderer against its own
// description instead of against a sentence somebody typed.
QVariantMap WorldApi::tierTable()
{
    static const char *kTechniqueNames[] = { "off", "vct", "vct_pcc_hybrid" };
    static const char *kQualityNames[]   = { "low", "medium", "high" };

    QVariantList photonList;
    for (const QString &name : worldmodes::photonTierNames()) {
        bool ok = false;
        const worldmodes::PhotonTier t = worldmodes::photonTierFromName(name, &ok);
        if (!ok) continue;
        const int quality = qBound(0, worldmodes::photonQuality(t), 2);
        const jahshaka::engine::GiQualityFacts facts =
            jahshaka::engine::giQualityFacts(jahshaka::engine::GiQuality(quality));

        // ...AND THE VR COLUMN OF THE SAME TABLE (lane V1-RIG item 4): what a
        // HEADSET's chain is, which is not the same chain — five times the
        // pixels for half the frame (GiViewProfile carries the measurement).
        const jahshaka::engine::GiQualityFacts vrFacts =
            jahshaka::engine::giQualityFacts(jahshaka::engine::GiQuality(quality),
                                             jahshaka::engine::GiViewProfile::Vr);

        auto chainOf = [&](const jahshaka::engine::GiQualityFacts &f) {
            QVariantList rows;
            if (!worldmodes::photonCascades(t)) return rows;
            // THE STEPS RESOLVED, not the tier row's zeros (CASCADE-STEP-1).
            // `giResolveCascadeSteps` is the SAME function the renderer's
            // resolveCascadeTable() calls, so this chain is the chain that
            // would be built, down to the near-field guarantee on each row —
            // which is the whole point of the verb.
            jahshaka::engine::GiParams::GiCascadeDesc resolved[4];
            for (int i = 0; i < f.cascadeCount && i < 4; ++i) resolved[i] = f.cascades[i];
            jahshaka::engine::giResolveCascadeSteps(resolved, std::min(f.cascadeCount, 4));
            for (int i = 0; i < f.cascadeCount && i < 4; ++i)
                rows.append(QVariantMap{
                    { QStringLiteral("halfSize"), double(resolved[i].halfSize) },
                    { QStringLiteral("resolution"), resolved[i].resolution },
                    { QStringLiteral("stepCells"), double(resolved[i].stepCells) },
                    { QStringLiteral("cell"),
                      double(jahshaka::engine::giCascadeCell(resolved[i])) },
                    { QStringLiteral("step"),
                      double(resolved[i].stepCells *
                             jahshaka::engine::giCascadeCell(resolved[i])) },
                    { QStringLiteral("guaranteedRadius"),
                      double(jahshaka::engine::giCascadeGuaranteedRadius(resolved[i])) },
                    { QStringLiteral("nearFieldRadius"),
                      double(jahshaka::engine::giNearFieldRadius(resolved[i])) } });
            return rows;
        };
        ...
        photonList.append(QVariantMap{
            ...
            { QStringLiteral("technique"),
              QLatin1String(kTechniqueNames[qBound(0, worldmodes::photonTechnique(t), 2)]) },
            { QStringLiteral("quality"), QLatin1String(kQualityNames[quality]) },
            { QStringLiteral("ddgi"), worldmodes::photonDdgi(t) },
            { QStringLiteral("bounces"), worldmodes::photonBounces(t) },
            { QStringLiteral("probeSize"), worldmodes::photonProbeSize(t) },
            { QStringLiteral("cascades"), worldmodes::photonCascades(t) },
    ...
    // Which Photon tier each WORLD mode selects — the `photon` row's own four
    // columns, reported by name so nobody has to map an ordinal.
    QVariantList worldList;
    const worldmodes::Row *photonRow = nullptr;
    for (const worldmodes::Row &r : worldmodes::rows())
        if (r.id == worldmodes::photonRowId()) photonRow = &r;
    const QStringList modeNames = worldmodes::modeNames();
    for (int i = 0; i < modeNames.size(); ++i) {
        const int column = photonRow ? photonRow->tier[i] : 0;
        worldList.append(QVariantMap{
            { QStringLiteral("mode"), modeNames[i] },
            { QStringLiteral("photon"),
              column <= 0 ? QStringLiteral("off")
                          : worldmodes::photonTierName(
                                worldmodes::PhotonTier(qBound(0, column - 1, 3))) } });
    }

    return QVariantMap{ { QStringLiteral("photon"), photonList },
                        { QStringLiteral("world"), worldList } };
}
