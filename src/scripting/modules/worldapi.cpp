...
QVector<VerbInfo> WorldApi::verbs() const
{
    return {
        ...
        { "shadows", "world.shadows({enabled, mapBudget}) -> bool",
          ...
          Needs::Document },
        { "gi", "world.gi({tier, mode, quality, bounces, dynamicProbes, light, boundsMin, boundsMax, autoBoundsMax, pccGrid, updateBudget, probeHdr, probeShadows, overlap, snapDeviation, snapSidesMin, snapSidesMax, rayMarchStepScale, ddgi, ddgiIntensity, ddgiAmbient, ddgiSource}) -> bool",
        ...
        { "rayon", "world.rayon({enabled, tier}) -> {enabled, tier, custom, deviations, technique, quality, ddgi, bounces, dynamicProbes, row, ddgiIntensity, ddgiAmbient, ddgiSource, updateBudget}",
          ...
          "'enabled' true|false turns it on and off. Off is the renderer's GI mode set to off and nothing else — no second flag to disagree with it — and the tier is remembered, so turning it back on restores the quality you had. 'tier' is low|medium|high|epic: low bounces one light off the scene (Instant Radiosity, no voxels, and emissive surfaces and area lights contribute nothing to it); medium voxelizes the lit volume and feeds the irradiance field from it, which REPLACES the cone-traced diffuse with probe-stored bounce that cannot leak through walls; high adds a grid of parallax-corrected reflection probes captured in HDR with shadows; epic adds three light bounces and two dynamic reflection probes a frame (the probes covering whatever moves re-capture every frame instead of waiting for the budget's sweep). New scenes are born epic. "
          "Called with no argument it reads. 'custom' is true when a setting you pinned deviates from what the tier would give it, and 'deviations' names those settings — the tier is still the tier, your pin still wins, and world.clearOverride({id}) hands one back (ids: giMode, giQuality, giDdgi, giBounces, giDynamicProbes, rayon). 'technique', 'quality', 'ddgi', 'bounces' and 'dynamicProbes' are what the tier and your pins RESOLVED to, in world.gi's spelling, and 'row' is the tier's own column set {technique, quality, ddgi, bounces, dynamicProbes} before any pin — the effective table row, so a script can see tier against resolution. "
        ...
        { "refreshGi", "world.refreshGi() -> bool",
          ...
          Needs::Document },
        { "shadowStatus", "world.shadowStatus() -> {live, resolution, maps, pssmSplits, focusedMaps, casters, budget, requestedBudget, atlasWidth, atlasHeight, atlasBytes, mapped:[{slot, node, static, dirty, pssm, passes}], unmapped:[guid], shadowPassesLastFrame, staticMapRendersLastFrame, reflectPassesLastFrame, probePassesLastFrame, reflectLampPassesLastFrame, probeLampPassesLastFrame, cachedInstances, uncachedInstances, viewCached, mapsDirtiedLastFrame}",
...
bool WorldApi::gi(const QVariantMap &params)
{
    ...
    static const QStringList known = {
        ...
        QStringLiteral("bounces"),   QStringLiteral("dynamicProbes"),
    ...
    if (params.contains("dynamicProbes")) {
        ...
        if (v < 0 || v > 8)
            return fail(QStringLiteral(
                ...
                "FRAME, on top of updateBudget, reserved for the probes covering whatever "
                "moved this frame (Rayon Epic's column: 2; the other tiers 0 = the sweep "
                "alone). Free while nothing moves."));
        scene->giDynamicProbes = v;
        worldmodes::pinRowValue(scene, QStringLiteral("giDynamicProbes"), v);
    }
    if (params.contains("light"))
        scene->giLightGuid = params.value("light").toString();
    if (params.contains("autoBoundsMax")) {
        const double v = params.value("autoBoundsMax").toDouble();
        if (v < 0.0 || v > 100000.0)
            return fail(QStringLiteral("world.gi: autoBoundsMax must be in [0, 100000] — the "
                                       "ceiling in METRES on the AUTOMATIC lit volume's largest "
                                       "axis, centred on the scene's content (64, the default, is "
                                       "half a metre per voxel at Epic's 128^3). 0 removes the "
                                       "ceiling; a pinned volume ignores it"));
        scene->giAutoBoundsMax = float(v);
    }
...
QVariantMap WorldApi::rayonRow(worldmodes::RayonTier tier)
{
    ...
    return QVariantMap{
        ...
        { QStringLiteral("dynamicProbes"), worldmodes::rayonDynamicProbes(tier) },
    };
}
...
QVariantMap WorldApi::rayonState(const iris::ScenePtr &scene)
{
    ...
    return QVariantMap{
        ...
        { QStringLiteral("dynamicProbes"), qBound(0, scene->giDynamicProbes, 8) },
...
QVariantMap WorldApi::giStatus()
{
    ...
    if (!st.available)
        return QVariantMap{ { QStringLiteral("mode"), requested },
                            ...
                            { QStringLiteral("dynamicProbes"), 0 },
                            { QStringLiteral("dynamicProbeUpdates"), 0 },
    ...
    return QVariantMap{ { QStringLiteral("mode"), st.mode },
                        ...
                        { QStringLiteral("dynamicProbes"), st.dynamicProbes },
                        { QStringLiteral("dynamicProbeUpdates"), st.dynamicProbeUpdates },
...
QVariantMap WorldApi::get()
{
    ...
    out["gi"] = QVariantMap{ { "mode", giModeNames[qBound(0, int(scene->giMode), 3)] },
                             ...
                             { "autoBoundsMax", scene->giAutoBoundsMax },
