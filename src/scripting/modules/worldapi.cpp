...
QVector<VerbInfo> WorldApi::verbs() const
{
    return {
        ...
        { "shadows", "world.shadows({enabled, mapBudget}) -> bool",
          ...
          Needs::Document },
        { "gi", "world.gi({tier, mode, quality, bounces, light, pccGrid, updateBudget, probeSize, probeHdr, probeShadows, overlap, snapDeviation, snapSidesMin, snapSidesMax, rayMarchStepScale, ddgi, ddgiIntensity, ddgiAmbient, ddgiSource}) -> bool",
...
bool WorldApi::gi(const QVariantMap &params)
{
    ...
    if (params.contains("cascades")) {
        scene->giCascades = params.value("cascades").toBool();
    }
...
QVariantMap WorldApi::giStatus()
{
    ...
    if (!st.available)
        return QVariantMap{ { QStringLiteral("mode"), requested },
                            ...
                            { QStringLiteral("probeEnclosedAxes"), 0 },
                            { QStringLiteral("probeGridRefused"), false },
    ...
    return QVariantMap{ { QStringLiteral("mode"), st.mode },
                        ...
                        { QStringLiteral("probeEnclosedAxes"), st.probeEnclosedAxes },
                        { QStringLiteral("probeGridRefused"), st.probeGridRefused },
...
QVariantMap WorldApi::rayTracing(const QString &mode)
{
    ...
    if (!mode.isEmpty()) {
        ...
        if (m != QLatin1String("auto") && m != QLatin1String("off")) {
            fail(QStringLiteral("world.rayTracing: '%1' is not a mode (expected \"auto\" or "
                                "\"off\")").arg(mode));
            return QVariantMap();
        }
        if (live) host.viewport->setRayTracing(m == QLatin1String("auto"));
    }
    if (live) rq = host.viewport->giStatus().rayQuery;
    // WHAT IT IS, NOT WHAT WAS ASKED FOR. `mode` comes back from the tier's own
    // `enabled`, so a machine that cannot trace reads "off" after being set to
    // "auto" — which is the honest answer and the one a suite must assert on.
    return QVariantMap{
        { QStringLiteral("mode"),
          rq.enabled ? QStringLiteral("auto") : QStringLiteral("off") },
