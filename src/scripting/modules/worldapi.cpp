...
QVector<VerbInfo> WorldApi::verbs() const
{
    return {
        ...
        { "setAtomView", "world.setAtomView('off'|'triangles'|'levels'|'buckets'|'objects') -> bool",
          ...
          "Objects the renderer keeps on the stock PBR shader keep their lit picture in every mode — transparent, two-sided, planar-mirror, alpha-tested and SKINNED (posed characters) objects among them; world.atomStatus() names why each one is there — and so does anything standing in front of a coloured surface. Nothing is painted where the split is off (world.setAtomDraw(false)) or in a VR headset (a stereo view draws everything through the stock shader). "
          "A viewing aid, not a setting: not saved, not undoable, switching rebuilds nothing, and opening or creating a scene starts 'off'. A screenshot taken while it is on shows it; the project thumbnail never does. Answers true when the view was accepted (false for an unknown name or with no engine viewport).",
          Needs::Document },
